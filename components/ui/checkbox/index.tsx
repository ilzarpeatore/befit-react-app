...
>(function IconWrapper({ ...props }, ref) {
  // @ts-ignore — el tipo de ref de styled(UIIcon) no coincide 1:1 con el de
  // UIIcon sin envolver (límite de complejidad estructural de TS), no afecta
  // al runtime.
...
// Se tipa contra StyledUIIcon (el componente concreto), no contra
// UICheckbox.Icon (el slot genérico de createCheckbox()) — ese genérico
// pierde size/height/width en esta versión alpha de @gluestack-ui/core.
// StyledUIIcon en sí mismo es un tipo unión (modo "icono SVG" vs modo
// "envuelve un componente propio"); size/height/width solo existen en una
// de las dos ramas, así que se añaden aquí sueltos para que el chequeo
// condicional de más abajo (que ya existía en el componente) compile sin
// cambiar el comportamiento en tiempo de ejecución.
type ICheckboxIconProps = React.ComponentPropsWithoutRef<typeof StyledUIIcon> &
