...
import { ScrollView, TextInput, Image, Alert, StyleSheet } from 'react-native';
...
export default function AddPostScreen({ navigation, route }: any) {
  ...
  return (
    <Box className="flex-1 bg-background">
      <Box style={{ paddingTop: 48, paddingBottom: 14 }} className="flex-row items-center justify-between px-4 bg-card">
        <Button variant="ghost" size="icon" onPress={() => navigation.goBack()}>
          <Icon name="chevron-back" size={24} className="text-foreground" />
        </Button>
        <Heading size="sm">{flow === 'EditFlow' ? 'Edit Post' : 'New Post'}</Heading>
        <Box className="w-10" />
      </Box>
      ...
      <ScrollView className="flex-1 px-4" showsVerticalScrollIndicator={false}>
        <VStack space="lg" style={{ paddingTop: 16, paddingBottom: 32 }}>
          {/* Description field */}
          <TextInput
            className="bg-card rounded-sm p-3.5 text-foreground font-gilroy-regular text-sm border border-border"
            style={{ minHeight: 140 }}
