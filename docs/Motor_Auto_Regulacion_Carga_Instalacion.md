...
El documento original llama a la tabla principal `progression_rules`. Esa tabla **ya existía en producción** (`2026_07_15_090002_create_progression_rules_table.php`) con un propósito completamente distinto: multiplicador de carga semanal *por programa* (semana → `load_multiplier`/`is_deload`), consumido por `TrainingProgramGeneratorService`, con 0 filas pero **en uso real**. Reutilizar el nombre habría chocado con datos de producción de un feature ya activo. Se prefijó `session_` en todo el esquema nuevo para dejar la distinción explícita: la tabla nueva actúa *sesión a sesión* sobre el auto-regulador de carga; la vieja actúa *semana a semana* sobre la generación de programas.
